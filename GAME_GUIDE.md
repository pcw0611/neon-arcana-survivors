...
<td>🔫 <b>실버불렛</b></td>
<td>★★☆☆☆</td>
<td>성좌탄이 은탄으로 변경. 부채꼴 대신 <b>가장 가까운 적 한 방향으로 계속 쏟아붓는 연사</b> (탄 크기 대폭 축소, 속도 2배, 날아갈수록 가속)</td>
<td>자신을 중심으로 사방에 무작위 난사, 한계돌파할수록 탄 수 증가</td>
</tr>
<tr>
<td>🥷 <b>쉐도우마스터</b></td>
<td>★★☆☆☆</td>
...
기본 실루엣은 전부 동일한 그림자 몹이며, 색상 글로우와 아이콘으로 종류를 구분한다. 시간이 지날수록 새 종류가 아래 순서로 순차 등장하되, 각 종류가 등장 직후 얼마 지나지 않아 다음 종류가 합류하도록 간격을 좁혀뒀다 — 특정 구간에 한 종류만 몰려나오는 느낌을 피하기 위함(예: 10분 시점 기준 스토커 약 50%, 나머지 5종이 골고루 나머지 50%를 나눠 가짐).
...
| 2 | **차저** (돌진) | 약 1분 50초~ | <img src="docs/guide/icon-charger.png" width="28"> | 접근 후 예고 → 빠르게 돌진 |
| 3 | **워더** (쉴드) | 약 3분 50초~ | <img src="docs/guide/icon-warder.png" width="28"> | 체력의 75%에 해당하는 쉴드 보유 (광검에 특효, 성좌탄에 약함) |
| 4 | **봄버** (자폭) | 약 5분 40초~ | 없음(전용 드론 스프라이트) | 접근 후 자폭, 광역 피해 |
| 5 | **스플리터** (분열) | 약 7분 20초~ | 없음 | 사망 시 약한 새끼 몹 2마리로 분열 |
...
보스 고유 패턴 위에, 유물처럼 이름과 등급이 있는 무작위 **옵션**이 추가로 붙는다. 게임 시작 시 1개에서 시작해 약 3분마다 1개씩 늘어나 12분 이후로는 항상 5개가 붙으며, 게임이 길어질수록(장기전) 높은 등급의 옵션이 나올 확률도 함께 올라간다. 보스 HUD 이름 옆에 현재 붙은 옵션이 등급 색상과 함께 표시된다.

| 옵션 | 등급 | 효과 |
|---|---|---|
| 신속 | 일반 | 이동 속도 +18% |
| 불안정 | 일반 | 패턴 재사용 대기시간 -15% |
| 추적자 | 레어 | 주기적으로 유도탄 3발 발사 |
| 메아리 | 레어 | 주기적으로 방사형 탄막 추가 발사 |
| 중장갑 | 유니크 | 성좌탄 피해 감소(광검엔 오히려 약함) |
| 과부하 | 유니크 | 모든 패턴 재사용 대기시간 -22% |
| 지뢰밭 | 전설 | 주기적으로 공중폭격 다수 추가 소환 |
| 재생 | 신화 | 초당 최대 체력 1.5% 회복 |
| 감전 오라 | 신화 | 주기적으로 근접 범위에 감전 피해 |

15분 이후부터는 낮은 확률로 기존 보스가 아직 살아있는 상태에서 **두 번째 보스**가 추가로 등장할 수 있다 (HUD에 "2체 감지" 표시).
...
랭킹 관리자 페이지(`/admin`)와 동일한 비밀번호로 접근하는 `/test` 경로에 치트 콘솔이 있다. 실제로 플레이하지 않고도 빌드·밸런스를 빠르게 검증하기 위한 내부 QA 도구이며, 일반 플레이어에게는 노출되지 않는다.

![테스트 콘솔 로그인 화면](docs/guide/test-login.png)

로그인하면 실제 게임 화면과 치트 패널이 나란히 뜬다. 왼쪽에서 평소처럼 게임을 시작한 뒤, 오른쪽 패널로 아래 항목들을 즉시 조작할 수 있다.

![테스트 콘솔 화면](docs/guide/test-console.png)

- **레벨 / 시간**: 특정 레벨이나 경과 시간으로 즉시 점프
- **체력**: HP 직접 설정, 완전 회복, 무적 모드 on/off
- **몬스터 / 보스**: 종류·수량을 지정해 소환, 보스 즉시 소환 / 즉사, 몹 전체 제거
- **클래스**: 전직 강제 적용 (단, 런당 1회 제한은 그대로 적용되어 중복 적용은 막힘)
- **능력 / 유물**: 원하는 강화나 유물을 랭크·레벨을 지정해 즉시 부여 (선행 조건은 무시하고 강제로 부여됨)
- **커스텀 스크립트**: 게임 상태 객체 `S`를 인자로 받는 임의의 JS 표현식을 실행 — 세밀한 디버깅용

새 밸런스나 이펙트를 넣을 때마다 매번 처음부터 플레이해서 조건을 맞출 필요 없이, 이 콘솔로 원하는 상황(특정 레벨, 특정 유물 조합, 몰려있는 몹 등)을 몇 초 안에 재현해 확인하는 용도로 쓴다.
